...
class VGG19(object):
    ...
    def load_image(self, image_dir, image_height, image_width):
        ...
        # 调整图像大小
        self.input_image = input_image.resize((image_height, image_width))

        # 获取图像的形状
        image_shape = self.input_image.size + (3,)
    ...
    def save_image(self, input_image, image_shape, image_dir):
        ...
        if len(image_shape) == 3:
            image_shape = (image_shape[1], image_shape[0])  # 交换宽度和高度
        # 调整图像大小（使用默认的高质量重采样滤波器）
        pillow_image = pillow_image.resize(image_shape)

        # 保存图像
        pillow_image.save(image_dir)
...
if __name__ == '__main__':
    ...
    adam_optimizer = AdamOptimizer(1.0, [1, 3, IMAGE_WIDTH, IMAGE_HEIGHT])
