...
def vgg19():
    ...
    for i, layer_name in enumerate(layers):
        if layer_name.startswith('conv'):
            # TODO: 在时序容器中传入卷积运算
            in_ch, out_ch = conv_map[layer_name]
            layer_container.add_module(layer_name, nn.Conv2d(in_channels=in_ch, out_channels=out_ch, kernel_size=3, padding=1))
            in_channels = out_ch
            ...
            # TODO: 在时序容器中执行全连接层计算
            layer_container.add_module(layer_name, nn.Linear(in_features=25088, out_features=4096))
        elif layer_name == 'fc7':
            # TODO: 在时序容器中执行全连接层计算
            layer_container.add_module(layer_name, nn.Linear(in_features=4096, out_features=4096))
        elif layer_name == 'fc8':
            # TODO: 在时序容器中执行全连接层计算
            layer_container.add_module(layer_name, nn.Linear(in_features=4096, out_features=num_classes))
...
if __name__ == '__main__':
    ...
    st = time.time()
    #TODO: 进行推理，得到prob
    prob = net_trace(input_image).cpu()
