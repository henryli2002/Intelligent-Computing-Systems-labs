...
if __name__ == '__main__':
    ...
    s1, s2, s3, s4 = [get_gram_matrix(s).detach() for s in [s1, s2, s3, s4]]
    j = 0
    count = 0
    epochs = 0
    while j <= epochs:
        for i, image in enumerate(data_loader):
            ...
            out1, out2, out3, out4 = mlu_net(mlu_image_c)
            ##############计算风格损失#################
            ...
            if i % 10 == 0:
                ...
                images = torch.cat([mlu_image_g, mlu_image_c], dim=0)
                save_image(images, f"./out/train_mlu/image{j}_{i}.jpg")
        j += 1
